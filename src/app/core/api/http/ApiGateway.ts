...
import {Response, RequestOptions, RequestMethod, URLSearchParams, Headers, Http, HttpModule} from "@angular/http";
...
export class ApiGateway {
    ...
    private interpolateUrl(options: ApiGatewayOptions): ApiGatewayOptions {
        var isHttp = options.url.toLowerCase().indexOf('http://') === 0;
        var isHttps = options.url.toLowerCase().indexOf('https://') === 0;
        var isDefaultProtocol = options.url.toLowerCase().indexOf('//') === 0;
...
@NgModule({
    imports: [CommonModule, HttpModule, CookieService],
    providers: [ApiGateway],
    exports: [ApiGateway]
})
export class OEHttpModule {
}