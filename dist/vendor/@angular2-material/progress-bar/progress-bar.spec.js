...
testing_1.describe('MdProgressBar', function () {
    ...
    testing_1.it('should return the transform attribute for bufferValue and mode', function (done) {
        builder
            ...
            var progressElement = fixture.debugElement.query(platform_browser_1.By.css('md-progress-bar'));
            var progressComponent = progressElement.componentInstance;
            testing_1.expect(progressComponent._primaryTransform()).toEqual({ transform: 'scaleX(0)' });
            testing_1.expect(progressComponent._bufferTransform()).toBe(undefined);
            progressComponent.value = 40;
            testing_1.expect(progressComponent._primaryTransform()).toEqual({ transform: 'scaleX(0.4)' });
            testing_1.expect(progressComponent._bufferTransform()).toBe(undefined);
            progressComponent.value = 35;
            progressComponent.bufferValue = 55;
            testing_1.expect(progressComponent._primaryTransform()).toEqual({ transform: 'scaleX(0.35)' });
            testing_1.expect(progressComponent._bufferTransform()).toBe(undefined);
            progressComponent.mode = 'buffer';
            testing_1.expect(progressComponent._primaryTransform()).toEqual({ transform: 'scaleX(0.35)' });
            testing_1.expect(progressComponent._bufferTransform()).toEqual({ transform: 'scaleX(0.55)' });
            progressComponent.value = 60;
            progressComponent.bufferValue = 60;
            testing_1.expect(progressComponent._primaryTransform()).toEqual({ transform: 'scaleX(0.6)' });
            testing_1.expect(progressComponent._bufferTransform()).toEqual({ transform: 'scaleX(0.6)' });
            done();
        });
    });
});
